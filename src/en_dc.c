                           ...
                           const void *src_ptr, size_t src_len)
{
    ...
    while (src_read_ptr < src_end_ptr) {
        ...
        /* A COBS code byte cannot be zero */
        if (len_code == 0u) {
            result.status |= DECODE_ZERO_BYTE_IN_INPUT;
            break;
        }

        /* Number of bytes following this code byte */
        copy_len = (size_t)(len_code - 1u);

        remaining_input = (size_t)(src_end_ptr - src_read_ptr);
        ...
        /*
         * Insert the zero that was removed during encoding.
         * Do not insert one after the final block or after
         * a 0xFF block.
         */
        if ((len_code != 0xFFu) &&
